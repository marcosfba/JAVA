	...
	void btnBuscarClick(ActionEvent event) throws SQLException {
	   	if (isValidConsulta()) {
			...
			if (rbNome.isSelected()) {
			   dados.addAll(pessoaFisicaCtr.findAll("select p.* from tbPessoa p"," where p.nome like  '%"+ txtBuscar.getText() + "%'"));			
			} else if (rbEmail.isSelected()) {
				dados.addAll(pessoaFisicaCtr.findAll("select p.* from tbPessoa p"," where p.email like  '%"+ txtBuscar.getText() + "%'"));
			} else if (rbCPF.isSelected()) {
				dados.addAll(pessoaFisicaCtr.findAll("select p.* from tbPessoa p"," where p.Cpf like  '%"+ txtBuscar.getText() + "%'"));
			} else if (rbIdentidade.isSelected()) {
				dados.addAll(pessoaFisicaCtr.findAll("select p.* from tbPessoa p"," where p.Identidade like  '%"+ txtBuscar.getText() + "%'"));
			}
