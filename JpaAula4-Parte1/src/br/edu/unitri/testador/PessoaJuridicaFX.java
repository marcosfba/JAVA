	...
	void btnBuscarClick(ActionEvent event) throws SQLException {
	   	if (isValidConsulta()) {
			...
			if (rbNome.isSelected()) {
			   dados.addAll(pessoaJuridicaCtr.findAll("select p.* from tbPessoa p"," where p.nome like  '%"+ txtBuscar.getText() + "%'"));			
			} else if (rbEmail.isSelected()) {
				dados.addAll(pessoaJuridicaCtr.findAll("select p.* from tbPessoa p"," where p.email like  '%"+ txtBuscar.getText() + "%'"));
			} else if (rbCNPJ.isSelected()) {
				dados.addAll(pessoaJuridicaCtr.findAll("select p.* from tbPessoa p"," where p.cnpj like  '%"+ txtBuscar.getText() + "%'"));
			} else if (rbInscricao.isSelected()) {
				dados.addAll(pessoaJuridicaCtr.findAll("select p.* from tbPessoa p"," where p.InscEstadual like  '%"+ txtBuscar.getText() + "%'"));
			}
