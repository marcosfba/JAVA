...
public class PessoaFX implements Initializable {
	   	...
	   	if (isValidConsulta()) {
			...
			if (rbNome.isSelected()) {
			   dados.addAll(pessoaCtr.findAll("select p.* from tbPessoa p"," where p.nome like  '%"+ txtBuscar.getText() + "%'"));			
				...
				dados.addAll(pessoaCtr.findAll("select p.* from tbPessoa p, tbEndereco e, Pessoa_Endereco pe"," where pe.endereco_id = e.id and pe.pessoa_id = p.id and e.cidade like  '%"+ txtBuscar.getText() + "%'"));
			} else if (rbUF.isSelected()) {
				dados.addAll(pessoaCtr.findAll("select p.* from tbPessoa p, tbEndereco e, Pessoa_Endereco pe"," where pe.endereco_id = e.id and pe.pessoa_id = p.id and e.estado like  '%"+ txtBuscar.getText() + "%'"));
			}
	...
	void btnIncluirClick(ActionEvent event) {
		if (isValidaTela()) {
			Pessoa pessoa = new Pessoa(txtNome.getText(), txtEmail.getText(), ConverterUtil.localDateToDate(dtNascimento.getValue())); 
			switch (getOperacao()) {
			case 0:
				try {
